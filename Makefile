...
SCDOC=scdoc
DOCS = wvkbd.1
TEST_BIN = tests/test-mod-swipe tests/test-glide tests/test-keyboard-glide tests/test-main-glide-release tests/bench-glide
...
test-sanitize:
	$(MAKE) clean
	$(MAKE) TEST_CFLAGS='-std=c11 -Wall -Wextra -Werror -I. -g -fsanitize=address,undefined' TEST_LDFLAGS='-fsanitize=address,undefined' tests/test-mod-swipe tests/test-glide
