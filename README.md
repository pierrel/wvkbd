...
to `C-…` or `M-…` when the gesture has qualified. The threshold is 40% of the
key height, with a 12-pixel minimum, and vertical movement must be at least twice
the horizontal movement. A horizontal or diagonal movement reaching the
threshold cancels the key.
