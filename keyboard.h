...
enum kbd_candidate_owner {
    KbdCandidateOwnerNone = 0,
    KbdCandidateOwnerTouch,
    KbdCandidateOwnerPointer,
};
...
struct key {
    ...
    // actual coordinates on the surface (pixels), will be computed
    // automatically for all keys
    uint32_t x, y, w, h;
};
...
struct layout {
    ...
    const char *name;
    bool abc; // is this an alphabetical/abjad layout or not? (i.e. something
              // that is a primary input layout)
    uint32_t keyheight; // absolute height (pixels)
};

struct kbd_candidate_session {
    size_t count;
    struct glide_match matches[GLIDE_MAX_MATCHES];
    uint8_t case_mods;
    enum kbd_candidate_owner owner;
    size_t pressed_slot;
    bool pressed_inside;
    int32_t touch_id;
...
void kbd_activate_key(struct kbd *kb, struct key *k, uint32_t time,
...
enum kbd_candidate_event kbd_candidate_touch_down(struct kbd *kb, int32_t id,
                                                  int32_t x, int32_t y);
enum kbd_candidate_event kbd_candidate_touch_motion(struct kbd *kb, int32_t id,
                                                    int32_t x, int32_t y);
enum kbd_candidate_event kbd_candidate_touch_up(struct kbd *kb, int32_t id,
                                                uint32_t time);
enum kbd_candidate_event kbd_candidate_pointer_button(struct kbd *kb,
                                                      uint32_t button,
                                                      bool pressed, int32_t x,
